  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col">
      ...
      <main className="flex-grow max-w-2xl mx-auto px-6 py-8">
        <motion.h2 initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-3xl font-bold text-gray-800 mb-6">
          Track Your Package
        </motion.h2>
        <form onSubmit={handleTrack} className="flex gap-4 mb-8">
          <Input placeholder="Booking ID (e.g., 12345)" value={trackingId} onChange={(e) => setTrackingId(e.target.value)} />
        ...
        {trackingResult && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white p-6 rounded-lg shadow-md">
            <h3 className="text-lg font-semibold mb-4">Booking #{trackingResult.id}</h3>
            <div className="space-y-4">
              {trackingResult.statuses.map((s: any, i: number) => (
                <div key={i} className="flex items-center gap-4">
                  {s.status === "Delivered" ? <CheckCircle className="h-5 w-5 text-green-600" /> :
                   s.status === "Picked Up" ? <Truck className="h-5 w-5 text-blue-600" /> :
                   s.status === "In Transit" ? <MapPin className="h-5 w-5 text-orange-600" /> :
